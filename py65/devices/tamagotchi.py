...
class gpmem:
	def __setitem__(self, k, value):
		...
		self.a[k] = value
		if (k == 0x3000):
			f = open("C:\\Users\\Natalie\\Documents\\p" + str(v) + ".bin", 'rb')
			t = f.read()
...
class MPU:
    ...
    def __init__(self, memory=None, pc=0xCC00):
        # config
        self.name = '6502'
	...
	self.log = open("C:\\Users\\Natalie\\Documents\\logmcu.txt", 'w')
	self.loge = False
	self.log.close()
        # vm status
        self.excycles = 0
    ...
    def step(self):
	if( self.pc != (self.ppc + 1) ):
		if self.loge:
			self.log = open("C:\\Users\\Natalie\\Documents\\logmcu.txt", 'a')
			self.log.write(hex(self.memory[0x29]) + hex(self.pc))
			self.log.write("\n")
			self.log.close()
        if(self.pc == 0x4000):
		print "state " + hex(self.memory[0x22]) 
	self.ppc = self.pc
    ...
    def inter(self, loc):
        ...
        self.stPushWord((self.pc) & self.addrMask)
	print "pushing " + str((self.pc - 1) & self.addrMask)
	self.stPush(self.p)
	print "pushing " + str(self.p)
        self.pc = loc
